...
| 문항 1 | Deadlock PID 생존 + 로그 정지 | 충족 | PID `10964` 생존, 로그 크기 `2693 -> 2693 bytes`, `Shared_Memory_A` ↔ `Socket_Pool_B` 순환 대기 | `docs/issues/03_deadlock.md`, `evidence/deadlock/before_ps.txt`, `evidence/graphs/03_deadlock_wait_for_graph.svg` |
...
| 문항 3 | Deadlock 상호 배제/순환 대기 설명 | 충족 | `Shared_Memory_A`와 `Socket_Pool_B`의 A→B, B→A 대기 설명 | `docs/issues/03_deadlock.md`, `docs/evaluation_answers.md`, `evidence/graphs/03_deadlock_wait_for_graph.svg` |
